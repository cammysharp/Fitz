...
print('-------------------------------------------')
rank_range=[(rank-1)*int(nbins/8),rank*int(nbins/8)]
...
for i in range(rank_range[0],rank_range[1]):
   ...
   ##print(np.any(logLam1[goodpixels]==logLam1[i]))
       if np.any(logLam1[gp]==logLam1[j])==True:
...
plt.show()
plt.plot(lamGal,pp.gas_bestfit)
print(len(x_bar),log_gal[0,:].size,log_gal.shape)

f = h5py.File(f'C:/Users/cammy/OneDrive/Documents/astronomy/ppxf/ppxf_gas_out_NGC4694_{rank}.h5','w')
